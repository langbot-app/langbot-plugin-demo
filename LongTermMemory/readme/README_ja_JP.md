...
- `!memory export` コマンドで全 L1 プロフィールを JSON として出力できます
...
- **エクスポート（L1 プロフィール）：** `!memory export` コマンドで、すべてのセッションプロフィールとスピーカープロフィールを JSON として出力できます。コピーして保存し、バックアップや移行に使えます。
